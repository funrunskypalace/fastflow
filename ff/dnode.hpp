 ...
 */
     ...
     */
        ...
        {
            if (skipdnode || !P)
                return ff_node::Push(ptr, retry, ticks);
            return push(ptr);
        }
        ...
        {
            if (skipdnode || P)
                return ff_node::Pop(ptr, retry, ticks);
            return pop(ptr);
        }
     ...
     */
                 ...
                 const bool p, const int nodeId = -1, dnode_cbk_t cbk = 0)
        {
            ...
            ff_dnode::cb = cbk;
            if (P)
                ff_node::create_output_buffer(1, true);
            else
                ff_node::create_input_buffer(1);
