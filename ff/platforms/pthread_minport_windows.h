...
/* **************************************************************************
Ultra-minimal and incomplete compatibility layer for pthreads on Windows
platform. Errors are not managed at all. Designed to enable a basic porting of
FastFlow on Windows platform, if you plan to extensively use pthread consider to
install a full pthread port for windows. Some of the primitives are inspired to
Fastflow queue remix by Dmitry Vyukov http://www.1024cores.net/

March 2011 - Ver 0: Basic functional port, tested on Win 7 x64 - Performance not
yet extensively tested.
...
#define PTHREAD_CANCEL_ENABLE                                                  \
    0x01 /* Cancel takes place at next cancellation point */
#define PTHREAD_CANCEL_DISABLE 0x00 /* Cancel postponed */
#define PTHREAD_CANCEL_DEFERRED                                                \
    0x02 /* Cancel waits until cancellation point                              \
          */
                          ...
                          void* (*start_routine)(void*), void RESTRICT* arg)
{
    ...
    thread[0] = (HANDLE)_beginthreadex(
        0, 0, (unsigned(__stdcall*)(void*))start_routine, arg, 0, 0);
    return (0);
}
...
// This requires #define _WIN32_WINNT 0x0403 to be stated as C++ preprocessor
// option (-D_WIN32_WINNT=0x0403) MA: 26/04/14 this seems no longer true
...
INLINE pthread_t pthread_self(void) { return (GetCurrentThread()); }
